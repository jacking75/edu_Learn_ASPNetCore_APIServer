...
public class Login : ControllerBase
{
    ...
    public async Task<PkLoginResponse> Post(PkLoginRequest request)
    {
        ...
        using (var db = await DBManager.GetGameDBQuery())
        {
            ...
            Logger.ZLogInformation($"[Request Login] Email:{request.Email}, request.Password:{request.Password},  saltValue:{userInfo.SaltValue}, hashingPassword:{hashingPassword}");
