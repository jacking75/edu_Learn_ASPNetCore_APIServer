...
public class GameDb : IGameDb
{
    ...
    public async Task<Tuple<ErrorCode, IEnumerable<ModelGameDB.CharacterInfo>>> GetCharacterList(Int64 accountId)
    {
        try
        {
            var characterList = await _queryFactory.Query("character")
                ...
                .GetAsync<ModelGameDB.CharacterInfo>();

            return new Tuple<ErrorCode, IEnumerable<ModelGameDB.CharacterInfo>>(ErrorCode.None, characterList);
        }
        catch (Exception e)
        {
            _logger.ZLogError(e,
                $"[GameDb.GetCharacterList] ErrorCode : {ErrorCode.GetCharacterListFail}");
            return new Tuple<ErrorCode, IEnumerable<ModelGameDB.CharacterInfo>>(ErrorCode.GetCharacterListFail, null);
        }
    }
